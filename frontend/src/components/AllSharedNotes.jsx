  ...
  const deleteSharedNote = async (sharedNoteId) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const updatedSharedNotes = sharedNotes.filter(note => note._id !== sharedNoteId);
      ...
      <div className="w-full flex flex-col items-center bg-gray-100 gap-1 py-8 min-h-screen">
        {sharedNotes.length > 0 ? (
          sharedNotes.map((sharedNote) => (
            <div key={sharedNote._id} className="bg-white p-4 m-2 rounded-md w-[300px] sm:w-[500px] md:w-[600px] shadow-sm border hover:shadow-md transition-shadow">
              <div className="flex flex-col gap-3">
                {/* Note Content */}
                <div className="flex-1">
                  <div className="mb-3">
                    <h3 className="text-sm font-semibold text-gray-700 mb-2">Received Note:</h3>
                  ...
                  <div className="text-xs sm:text-sm text-gray-600 space-y-1 bg-gray-50 p-3 rounded-md">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-gray-700">📧 Shared from:</span> 
                      <span className="text-blue-600 font-medium">{sharedNote.sharedFrom}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-gray-700">📅 Received on:</span> 
                      <span>{new Date(sharedNote.createdAt).toLocaleString()}</span>
                    </div>
          ...
          <div className="bg-white p-8 rounded-md shadow-sm text-center text-gray-500 w-[300px] sm:w-[500px] md:w-[600px]">
            ...
            <p className="text-gray-500 mb-4">You haven't received any shared notes yet.</p>
            <div className="space-y-2">
              ...
              <div className="text-xs text-gray-400 mt-2">
                Ask someone to share a note with you, or share your notes with others
              </div>
            </div>
          </div>
        )}
